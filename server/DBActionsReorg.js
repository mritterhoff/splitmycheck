...
// get a rows x cols array of false's
function getFalseBooleanArray(rows, cols) {
  return [...Array(rows).keys()].map(r => new Array(cols).fill(false))
}
...
class DBActionsReorg {
  ...
  // get a 6char random link code that hasn't isn't in the db yet
  async getUnusedLinkCode() {
    let candidate;
    await this._checkReady().then(async () => {
      let tries = 10;
      while (tries-- > 0) {
        candidate = randomstring.generate(6);
  ...
  assembleObjFromLinkCode(link_code) {
    // 1. look up the link_code in the Splits table,
    this._checkReady().then(async () => {
  ...
  makeNewSplit(splitObj) {
    let link_code;
    this._checkReady()
      .then(() => this.getUnusedLinkCode())
      .then(async (unusedLinkCode) => {
    ...
    // TODO do something with the linkcode so we can return to the user
  }
...
const someDB = new DBActionsReorg();
// someDB.makeNewSplit(splitObj);

someDB.assembleObjFromLinkCode('xtHBuZ');
